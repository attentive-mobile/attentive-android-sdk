...
class RecordEventCallbackTest {
    ...
    @Test
    fun oldApi_bothCallbacksInvoked_doesNotCrash() = runBlocking {
    ...
    @Test
    fun oldApi_onlyFailureCalled_completesNormally() = runBlocking {
        // Arrange
        val tracker = createTrackerWithConfig(mockConfig)
    ...
    @Test
    fun oldApi_onlySuccessCalled_completesNormally() = runBlocking {
        // Arrange
        val tracker = createTrackerWithConfig(mockConfig)
    ...
    @Test
    fun oldApi_callbackInvokedMultipleTimes_onlyResumesOnce() = runBlocking {
        // Arrange
        val tracker = createTrackerWithConfig(mockConfig)
    ...
    private fun createTrackerWithConfig(config: AttentiveConfig): AttentiveEventTracker {
        val tracker = AttentiveEventTracker::class.java.getDeclaredConstructor().apply {
            isAccessible = true
        }.newInstance()

        // Use reflection to set the config field directly
        val configField = AttentiveEventTracker::class.java.getDeclaredField("config").apply {
            isAccessible = true
        }
